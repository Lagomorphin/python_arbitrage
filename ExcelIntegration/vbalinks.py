from ExcelIntegration.xlfuncs import sql_to_xl, xl_to_sql, insert_lists, get_arrays_list
from AmazonSelling.tools import call_sql, con_postgres
from AmazonSelling.inventory import update_skus
import datetime, xlwings as xw
...
def displaydata_tosql_already():
    ...
    colsNames = ('fragile', 'polybag', 'expire', 'ingredients', 'discontinue', 'edit', 'non_joliet', 'sku') #The order matters, needs to match the SQL statement
    theData = []
...
def displaydata_checkskus():
    #Checks each asin in purchasedSh to see if a sku already exists for it in io.SKUs. If so, overwrite the sku in purchasedSh with the old sku from SQL.
    ...
    asins = [datumsRange.columns(headersRow.value.index('asin') + 1)(i).value for i in range(1, datumsRange.rows.count + 1)]
...
def displaydata_expandrows():
    #Scans the active sheet (will only actually activate on purchasedSh) for asins or wm_ids that were added manually, i.e. have no other data in their rows.
    #Fills in the what empty columns it can from Products_WmAz, and additionally from io.SKUs to get the column <my_name>.
...
#     shtName = xw.Range('A1').sheet.name
    ...
    for i in range(1, datumsRange.rows.count + 1):
        if sum(xcv is not None for xcv in datumsRange.rows(i).value) <= 2: #This row has at most 2 filled-in columns (assumed to be asin or wm_id)
            
            #If wm_id is filled in, but not asin, get the asin from Products_WmAz
            if wmidCol(i).value and not asinCol(i).value:
                asinCol(i).formula = call_sql(con, 'SELECT asin FROM "Products_WmAz" WHERE wm_id = %s', [wmidCol(i).value], 'executeReturn')[0]
            #If sku is filled in, but not asin, get the asin from io.SKUs
            elif skuCol(i).value and not asinCol(i).value:
                asinCol(i).formula = call_sql(con, 'SELECT asin FROM io."SKUs" WHERE sku = %s', [skuCol(i).value], 'executeReturn')[0]
                
            #Now, get the rest of the columns from Products_WmAz using the asin
            a, b = (None for _ in range(2))
            try:
                a = call_sql(con, sqlTxt.format('Products_WmAz', 'asin'), [asinCol(i).value], 'executeReturn', dictCur = True)[0]                
            except IndexError: #No asins need to be expanded
                pass
            try:
                b = call_sql(con, sqlTxt.format('SKUs', 'sku'), [skuCol(i).value], 'executeReturn', dictCur = True)[0]
            except IndexError: #No asins need to be expanded
                pass
...
def displaydata_expandrows_old():
    ...
    #Fills in the what empty columns it can from Products_WmAz, and additionally from io.Manual if the active sheet is manualSh
...
#     shtName = xw.Range('A1').sheet.name
    ...
    for i in range(1, datumsRange.rows.count + 1):
        if sum(xcv is not None for xcv in datumsRange.rows(i).value) <= 2: #This row has at most 2 filled-in columns (assumed to be asin or wm_id)
            ...
            try:
                aa = call_sql(con, sqlTxt1, [asinsCol(i).value], 'executeReturn', dictCur = True) #Fetch data for this asin from Products_WmAz
...
def displaydata_tosql_enroute():
    ...
    colsNames = ['received', 'cancelled', 'lost', 'notes', 'purchase_id'] #The order matters, needs to match the SQL statement
    theData = []
    for i in range(1, datumsRange.rows.count + 1):
        theData.append([datumsRange.columns(_headersRow.value.index(_g) + 1)(i).value for _g in colsNames])
...
def displaydata_tosql_instock():
    ...
    colsNames1 = ['labeled', 'packing', 'lost', 'pack_date', 'notes', 'purchase_id'] #The order matters, needs to match the SQL statement
    theData1 = []
    ...
    #I need to go through and turn any blank strings ('') into None. Postgres can't handle blank string in non-text columns.
    #I could do this inside the list compehension with a conditional, but that would be ugly...and faster
    for i, theRow in enumerate(theData1):
        for j, theColumn in enumerate(theRow):
            if theColumn == '': theData1[i][j] = None    
