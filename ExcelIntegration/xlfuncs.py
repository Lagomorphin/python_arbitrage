...
import itertools, re, datetime, xlwings as xw


def sql_to_xl(tbl, wkbk=None, sh='Sheet1', where=False):
    ...
    #The data part of each 2nd level tuple is itself a tuple, and transposed so the data pastes in as columns instead of rows
    datumsByCol = tuple((colIndexes[i], tuple((j[i],) for j in call_sql(con, sqlTxt, [], 'executeReturn')),) for i in range(len(colIndexes)))
    if con:
        con.close()
...
def insert_lists(datums, col, sh, wkbk=None):
    ...
    if len(datums) != datumsRange.rows.count:
        raise RuntimeError('xlfuncs.insert_lists: The length of the <datums> parameter is different from the number of data rows in the Excel sheet <{}>'.format(sh))
...
def get_arrays_list(sh, rng, dimTypes): #sh is only needed for error messages
    ...
    #Extract the data validation list from each cell
        ...
        for theCell in rng:
            try:
                ...
                #If there's no data validation list, just get whatever's in the cell anyways in case it's an expiration date
                v = theCell.value
                if v:
                    w = v.strip()
                    if w:
    ...
    for theRow, cellVal in enumerate(datums):
        if cellVal:
            ...
            for listVal in re.split(r'(?:\s*),(?:\s*)', cellVal): #Splits on ',' surrounded by any amount of whitespace (including no whitespace)
                elem = []
                
                #Loop through each dimension in the list value (separated by ':')
                for dimIndex, theDim in enumerate(re.split(r'(?:\s*):(?:\s*)', listVal)): #Same as above, but splits on ':'
...
def get_arrays_text(sh, rng, dimTypes): #sh is only needed for error messages
    ...
    The text in each Excel cell must be formatted with commas or semicolons separating elements. Colons add extra dimensions to each element.    
    Parameters: sh, rng, dimTypes (a list/tuple of the Python data types of each dimension the array elements)
    Returns: a list of lists representing the data from the entire column. Each inner list contains 1 postgres array.
    '''
    ...
    for theRow, cellVal in enumerate(datums):
        if cellVal:
            if cellVal.strip(): #In case there's a cell with nothing but whitespace, which we would just want to skip
                ...
                for theElem in re.split(r'(?:\s*)(?:,|;)(?:\s*)', cellVal): #Splits on ',' or ';' surrounded by any amount of whitespace (including no whitespace)
                    elem = []
                    
                    #Loop through each dimension in the element (separated by ':')
                    for dimIndex, theDim in enumerate(re.split(r'(?:\s*):(?:\s*)', theElem)): #Same as above, but splits on ':'
...
if __name__ == '__main__':
    ...
    sql_to_xl('SKUs', wkbk=xw.Book.caller(), sh='Sheet1', where='where discontinue = TRUE and enroute + stock + processing + available = 0')
#     test_get_arrays()
#     test_insert_lists()
