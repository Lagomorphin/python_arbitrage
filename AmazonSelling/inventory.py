...
from Amazon.mwsutils import Products, Reports, transfer_wm_datums, calc_column
...
def update_already():
    ...
    con = con_postgres()
    wmIds = tuple(q[0] for q in call_sql(con, 'SELECT wm_id FROM io."SKUs" WHERE discontinue IS NOT True', [], "executeReturn"))
    asins = tuple(q[0] for q in call_sql(con, 'SELECT asin FROM io."SKUs" WHERE discontinue IS NOT True', [], "executeReturn"))
    skus = tuple(q[0] for q in call_sql(con, 'SELECT sku FROM io."SKUs" WHERE discontinue IS NOT True', [], "executeReturn"))
    con.close()
